...
def chat_with_agent(query: str, payload: dict[str, Any] | None = None) -> str:
    ...
    prompt = (
        "You are the conversational layer for an AI knowledge workspace.\n"
        "When the user has no uploaded file or active context, respond like a normal helpful chatbot.\n"
        "Be direct, useful, and concise.\n"
        "If the user asks for a task that requires an uploaded file, transcript, or document, explain what is missing.\n\n"
        f"Context:\n{context_block}\n\n"
        f"User Query:\n{clean_query}\n\n"
        "Assistant Response:"
    )
